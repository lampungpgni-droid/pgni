...
// Cek role - hanya super_admin dan admin yang bisa tambah user
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_permissions = isset($_POST['permissions']) ? $_POST['permissions'] : [];
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            if (empty($error)) {
                ...
                if (mysqli_query($conn, $query)) {
                    ...
                    // Simpan permission per user
                    if (!empty($user_permissions) && $role !== 'super_admin') {
                        foreach ($user_permissions as $perm_id) {
                            $perm_id = (int)$perm_id;
                            $insert_perm = "INSERT INTO user_permissions (user_id, permission_id, granted) 
                                            VALUES ($new_user_id, $perm_id, 1)";
                    ...
                    if ($akses_semua == 1) {
                        mysqli_query($conn, "INSERT INTO user_wilayah_akses (user_id, akses_semua) VALUES ($new_user_id, 1)");
                    } else {
                        // Simpan kabupaten
                        if (!empty($kabupaten_terpilih)) {
                            foreach ($kabupaten_terpilih as $kab_id) {
                                $kab_id = (int)$kab_id;
                                mysqli_query($conn, "INSERT INTO user_wilayah_akses (user_id, kabupaten_id) VALUES ($new_user_id, $kab_id)");
                            }
                        }
                        ...
                        if (!empty($kecamatan_terpilih)) {
                            foreach ($kecamatan_terpilih as $kec_id) {
                                $kec_id = (int)$kec_id;
                                mysqli_query($conn, "INSERT INTO user_wilayah_akses (user_id, kecamatan_id) VALUES ($new_user_id, $kec_id)");
                            }
                        }
                        ...
                        if (!empty($desa_terpilih)) {
                            foreach ($desa_terpilih as $desa_id) {
                                $desa_id = (int)$desa_id;
                                mysqli_query($conn, "INSERT INTO user_wilayah_akses (user_id, desa_id) VALUES ($new_user_id, $desa_id)");
                            }
                        }
                    }
                    
                    header('Location: user.php?msg=tambah');
                    exit;
                } else {
...
<div class="page-header">
    <div class="page-header-left">
        ...
        <p class="text-muted">Tambahkan user baru untuk mengakses sistem</p>
    </div>
    ...
    <form action="" method="POST" id="formTambahUser">
        ...
        <div class="form-section" id="wilayahSection">
            ...
            <div class="form-section-body">
                    ...
                    <div class="form-control-wrap">
                        <label class="checkbox-toggle">
                            <input type="checkbox" id="akses_semua" name="akses_semua" value="1" 
                                   ...
                                   <?php echo isset($_POST['akses_semua']) ? 'checked' : ''; ?>>
                ...
                <!-- Pilihan Wilayah -->
                <div id="wilayah_container" style="<?php echo (isset($_POST['akses_semua'])) ? 'display:none;' : ''; ?>">
                    ...
                    <div class="wilayah-grid" id="kabupaten_grid">
                        <?php 
                        $selected_kab = isset($_POST['kabupaten']) ? $_POST['kabupaten'] : [];
                        mysqli_data_seek($kabupaten_list, 0);
                        while ($kab = mysqli_fetch_assoc($kabupaten_list)): 
                            $checked = in_array($kab['id'], $selected_kab);
                        ?>
