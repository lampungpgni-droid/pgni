...
// Ambil data guru
$query_guru = "SELECT g.*, k.nama as kecamatan_nama, d.nama as desa_nama 
               FROM guru_ngaji g 
...
// ============================================
// CEK AKSES UNTUK PETUGAS KECAMATAN
// ============================================
$user_role = $_SESSION['role'] ?? 'admin';
$user_kecamatan_id = $_SESSION['kecamatan_id'] ?? 0;

if ($user_role === 'petugas_kecamatan') {
    // Petugas hanya bisa edit guru di kecamatannya
    if ($guru['kecamatan_id'] != $user_kecamatan_id) {
        header('Location: guru_petugas.php?error=akses_ditolak');
        exit;
    }
    // Redirect ke guru_petugas setelah selesai
    $redirect_after = 'guru_petugas.php';
} else {
...
// Ambil daftar kecamatan berdasarkan kabupaten yang dipilih
$kabupaten_id = $guru['kabupaten_id'] ?? 0;
$query_kecamatan = "SELECT id, nama FROM kecamatan WHERE kabupaten_id = $kabupaten_id ORDER BY nama";
$kecamatan_list = mysqli_query($conn, $query_kecamatan);

// Ambil daftar desa berdasarkan kecamatan yang dipilih
$kecamatan_id = $guru['kecamatan_id'] ?? 0;
$query_desa = "SELECT id, nama FROM desa WHERE kecamatan_id = $kecamatan_id ORDER BY nama";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Hapus file flag
    $hapus_ktp = isset($_POST['hapus_ktp']) ? true : false;
    $hapus_kk = isset($_POST['hapus_kk']) ? true : false;
    ...
    // Cek NIK duplicate (kecuali dirinya sendiri)
    $check = mysqli_query($conn, "SELECT id FROM guru_ngaji WHERE nik = '$nik' AND id != $id");
    if (mysqli_num_rows($check) > 0) {
        echo json_encode(['status' => 'error', 'message' => 'NIK sudah terdaftar oleh guru lain!']);
        exit;
    }
    
    // Proses KTP
    $ktp_file = $guru['ktp_file'];
    if ($hapus_ktp) {
        // Hapus file KTP lama
        if ($ktp_file) {
            $old_path = $root_path . '/assets/images/ktp/' . $ktp_file;
            if (file_exists($old_path)) {
                unlink($old_path);
            }
        }
        $ktp_file = '';
    } elseif (!empty($ktp_base64)) {
    ...
    // Proses KK
    $kk_file = $guru['kk_file'];
    if ($hapus_kk) {
        // Hapus file KK lama
        if ($kk_file) {
            $old_path = $root_path . '/assets/images/kk/' . $kk_file;
            if (file_exists($old_path)) {
                unlink($old_path);
            }
        }
        $kk_file = '';
    } elseif (!empty($kk_base64)) {
        // Hapus file lama jika ada
        if ($kk_file) {
            $old_path = $root_path . '/assets/images/kk/' . $kk_file;
            if (file_exists($old_path)) {
...
<div class="page-header">
    <div class="page-header-left">
        <h2><i class="fas fa-user-edit"></i> Edit Guru Ngaji</h2>
        <p class="text-muted">Perbarui data guru ngaji yang terdaftar</p>
    </div>
...
<div class="loading-overlay" id="loadingOverlay">
    <div class="loading-content">
        ...
        <h3 id="loadingText">Memperbarui Data...</h3>
...
<div class="popup-modal" id="popupModal">
    <div class="popup-content">
        ...
        <p id="popupMessage">Data guru berhasil diperbarui.</p>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="nik" name="nik" 
                                   ...
                                   value="<?php echo htmlspecialchars($guru['nik']); ?>">
                        </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="no_telp" name="no_telp" 
                                   ...
                                   value="<?php echo htmlspecialchars($guru['no_telp']); ?>">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <select class="form-control" id="jenis_profesi" name="jenis_profesi">
                                ...
                                <option value="Guru Ngaji" <?php echo $guru['jenis_profesi'] == 'Guru Ngaji' ? 'selected' : ''; ?>>Guru Ngaji</option>
                                <option value="Marbot" <?php echo $guru['jenis_profesi'] == 'Marbot' ? 'selected' : ''; ?>>Marbot</option>
                                <option value="Ustadz/Ustadzah" <?php echo $guru['jenis_profesi'] == 'Ustadz/Ustadzah' ? 'selected' : ''; ?>>Ustadz/Ustadzah</option>
                                <option value="Pengajar TPA" <?php echo $guru['jenis_profesi'] == 'Pengajar TPA' ? 'selected' : ''; ?>>Pengajar TPA</option>
                                <option value="Pengajar MDTA" <?php echo $guru['jenis_profesi'] == 'Pengajar MDTA' ? 'selected' : ''; ?>>Pengajar MDTA</option>
                                <option value="Hafidz/Hafidzah" <?php echo $guru['jenis_profesi'] == 'Hafidz/Hafidzah' ? 'selected' : ''; ?>>Hafidz/Hafidzah</option>
                                <option value="Penjaga Makam" <?php echo $guru['jenis_profesi'] == 'Penjaga Makam' ? 'selected' : ''; ?>>Penjaga Makam</option>
                                <option value="Lainnya" <?php echo $guru['jenis_profesi'] == 'Lainnya' ? 'selected' : ''; ?>>Lainnya</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <select class="form-control" id="tempat_mengajar" name="tempat_mengajar" required>
                                ...
                                <option value="Rumah Pribadi" <?php echo $guru['tempat_mengajar'] == 'Rumah Pribadi' ? 'selected' : ''; ?>>🏠 Rumah Pribadi</option>
                                <option value="TPA (Taman Pendidikan Al-Qur'an)" <?php echo $guru['tempat_mengajar'] == 'TPA (Taman Pendidikan Al-Qur\'an)' ? 'selected' : ''; ?>>📖 TPA (Taman Pendidikan Al-Qur'an)</option>
                                <option value="MDTA (Madrasah Diniyah Takmiliyah)" <?php echo $guru['tempat_mengajar'] == 'MDTA (Madrasah Diniyah Takmiliyah)' ? 'selected' : ''; ?>>🕌 MDTA (Madrasah Diniyah Takmiliyah)</option>
                                <option value="Pondok Pesantren" <?php echo $guru['tempat_mengajar'] == 'Pondok Pesantren' ? 'selected' : ''; ?>>🕌 Pondok Pesantren</option>
                                <option value="Masjid/Musholla" <?php echo $guru['tempat_mengajar'] == 'Masjid/Musholla' ? 'selected' : ''; ?>>🕌 Masjid/Musholla</option>
                                <option value="Yayasan" <?php echo $guru['tempat_mengajar'] == 'Yayasan' ? 'selected' : ''; ?>>🏢 Yayasan</option>
                                <option value="Lainnya" <?php echo $guru['tempat_mengajar'] == 'Lainnya' ? 'selected' : ''; ?>>📌 Lainnya</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="tempat_mengajar_detail" name="tempat_mengajar_detail" 
                                   ...
                                   value="<?php echo htmlspecialchars($guru['tempat_mengajar_detail']); ?>">
                        </div>
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-control" id="bank" name="bank">
                                ...
                                <?php foreach ($bank_list as $b): ?>
                                    <option value="<?php echo $b; ?>" <?php echo $guru['bank'] == $b ? 'selected' : ''; ?>>
                                        <?php echo $b; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="no_rekening" name="no_rekening" 
                                   ...
                                   value="<?php echo htmlspecialchars($guru['no_rekening']); ?>">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <select class="form-control" id="kabupaten_id" name="kabupaten_id">
                                ...
                                ?>
                                    <option value="<?php echo $kab['id']; ?>" <?php echo $guru['kabupaten_id'] == $kab['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-control" id="kecamatan_id" name="kecamatan_id">
                                ...
                                <?php while ($kec = mysqli_fetch_assoc($kecamatan_list)): ?>
                                    <option value="<?php echo $kec['id']; ?>" <?php echo $guru['kecamatan_id'] == $kec['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-control" id="desa_id" name="desa_id">
                                ...
                                <?php while ($desa = mysqli_fetch_assoc($desa_list)): ?>
                                    <option value="<?php echo $desa['id']; ?>" <?php echo $guru['desa_id'] == $desa['id'] ? 'selected' : ''; ?>>
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; width: 100%; flex-wrap: wrap;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <span style="color: #555; font-size: 0.9rem; display: block;">
                                KTP: <code id="ktpFileNamePreview"><?php echo htmlspecialchars($guru['ktp_file'] ?: 'NAMA_NIK_ktp_timestamp.jpg'); ?></code>
                            </span>
                            <span style="color: #555; font-size: 0.9rem; display: block;">
                                KK: <code id="kkFileNamePreview"><?php echo htmlspecialchars($guru['kk_file'] ?: 'NAMA_NIK_kk_timestamp.jpg'); ?></code>
                            </span>
                    ...
                    <div class="form-control-wrap">
                        <div class="file-upload-wrapper">
                            ...
                            <div class="current-file">
                                <i class="fas fa-file-image" style="color: #d4a847;"></i>
                                <span>File saat ini: <strong><?php echo htmlspecialchars($guru['ktp_file']); ?></strong></span>
                                <label class="checkbox-inline" style="margin-left: 15px; cursor: pointer;">
                                    <input type="checkbox" name="hapus_ktp" value="1" id="hapus_ktp">
                                    <span style="color: #e74c3c;">Hapus file ini</span>
                                </label>
                                <br>
                                <a href="<?php 
                                    ...
                                    echo $protocol . $host . $path . '/assets/images/ktp/' . $guru['ktp_file']; 
                                ?>" target="_blank" style="color: #1a6e3a; font-size: 0.85rem;">
                ...
                <div class="form-group-row">
                    ...
                    <div class="form-control-wrap">
                        <div class="file-upload-wrapper">
                            ...
                            <div class="current-file">
                                <i class="fas fa-file-image" style="color: #d4a847;"></i>
                                <span>File saat ini: <strong><?php echo htmlspecialchars($guru['kk_file']); ?></strong></span>
                                <label class="checkbox-inline" style="margin-left: 15px; cursor: pointer;">
                                    <input type="checkbox" name="hapus_kk" value="1" id="hapus_kk">
                                    <span style="color: #e74c3c;">Hapus file ini</span>
                                </label>
                                <br>
                                <a href="<?php 
                                    ...
                                    $host = $_SERVER['HTTP_HOST'];
                                    $path = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'])), '/');
                                    echo $protocol . $host . $path . '/assets/images/kk/' . $guru['kk_file']; 
                                ?>" target="_blank" style="color: #1a6e3a; font-size: 0.85rem;">
                                    <i class="fas fa-external-link-alt"></i> Lihat file
                                </a>
                            </div>
                    ...
                    <div class="form-control-wrap">
                        <div class="input-group">
                            ...
                            <select class="form-control" id="status_verifikasi" name="status_verifikasi">
                                <option value="pending" <?php echo $guru['status_verifikasi'] == 'pending' ? 'selected' : ''; ?>>⏳ Pending</option>
                                <option value="disetujui" <?php echo $guru['status_verifikasi'] == 'disetujui' ? 'selected' : ''; ?>>✅ Disetujui</option>
                                <option value="ditolak" <?php echo $guru['status_verifikasi'] == 'ditolak' ? 'selected' : ''; ?>>❌ Ditolak</option>
                            </select>
                        </div>
                    </div>
                </div>
...
<style>
    ...
    .current-file .checkbox-inline {
        font-size: 0.85rem;
    }
    .current-file .checkbox-inline input[type="checkbox"] {
        width: 16px;
        height: 16px;
        cursor: pointer;
    }
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('../ajax/get_kecamatan.php?kabupaten_id=' + kabupatenId)
                ...
                .then(data => {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        ...
                        const selectedKec = <?php echo $guru['kecamatan_id'] ?? 0; ?>;
                        if (selectedKec > 0) {
            ...
            fetch('../ajax/get_desa.php?kecamatan_id=' + kecamatanId)
                ...
                .then(data => {
                    ...
                    if (Array.isArray(data) && data.length > 0) {
                        ...
                        // Set selected value jika ada
                        const selectedDesa = <?php echo $guru['desa_id'] ?? 0; ?>;
                        if (selectedDesa > 0) {
    ...
    // Trigger initial load untuk kecamatan dan desa
    ...
    document.getElementById('formGuru').addEventListener('submit', function(e) {
        ...
        const hapusKtp = document.getElementById('hapus_ktp') ? document.getElementById('hapus_ktp').checked : false;
        const hapusKk = document.getElementById('hapus_kk') ? document.getElementById('hapus_kk').checked : false;
        
        if (ktpBase64) {
            formData.append('ktp_base64', ktpBase64);
        }
        if (kkBase64) {
            formData.append('kk_base64', kkBase64);
        }
        if (hapusKtp) {
            formData.append('hapus_ktp', '1');
        }
        if (hapusKk) {
            formData.append('hapus_kk', '1');
        }
        ...
        .then(data => {
            ...
            setTimeout(() => {
                ...
                if (data.status === 'success') {
                    ...
                    showPopup('error', '❌ Gagal!', data.message || 'Terjadi kesalahan saat memperbarui data.');
                }
            }, 500);
        })
        .catch(error => {
    ...
    function showLoading(text, subText) {
        document.getElementById('loadingText').textContent = text || 'Memperbarui Data...';
    ...
    document.getElementById('resetBtn').addEventListener('click', function(e) {
        ...
        // Reset hapus checkbox
        if (document.getElementById('hapus_ktp')) {
            document.getElementById('hapus_ktp').checked = false;
        }
        if (document.getElementById('hapus_kk')) {
            document.getElementById('hapus_kk').checked = false;
        }
        ...
        document.getElementById('nik').value = '<?php echo htmlspecialchars($guru['nik']); ?>';
        document.getElementById('nama').value = '<?php echo htmlspecialchars($guru['nama']); ?>';
        document.getElementById('no_telp').value = '<?php echo htmlspecialchars($guru['no_telp']); ?>';
        document.getElementById('tempat_mengajar').value = '<?php echo htmlspecialchars($guru['tempat_mengajar']); ?>';
        document.getElementById('tempat_mengajar_detail').value = '<?php echo htmlspecialchars($guru['tempat_mengajar_detail']); ?>';
        document.getElementById('jenis_profesi').value = '<?php echo htmlspecialchars($guru['jenis_profesi']); ?>';
        document.getElementById('bank').value = '<?php echo htmlspecialchars($guru['bank']); ?>';
        document.getElementById('no_rekening').value = '<?php echo htmlspecialchars($guru['no_rekening']); ?>';
        document.getElementById('status').value = '<?php echo htmlspecialchars($guru['status']); ?>';
        document.getElementById('status_verifikasi').value = '<?php echo htmlspecialchars($guru['status_verifikasi']); ?>';
        
        // Trigger change untuk kabupaten
        const kabSelect = document.getElementById('kabupaten_id');
        kabSelect.value = '<?php echo $guru['kabupaten_id'] ?? 0; ?>';
        if (kabSelect.value) {
