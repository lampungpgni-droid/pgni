...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $author   = $_SESSION['nama_lengkap'] ?? 'Admin';
    ...
    // Proses Image Base64 & Pembuatan Direktori Otomatis
    $gambar = '';
    if (!empty($gambar_base64)) {
        ...
        if (!$gambar) {
            ob_clean();
            echo json_encode(['status' => 'error', 'message' => 'Gagal memproses gambar berita. Pastikan format gambar valid (JPG/PNG/WebP) dan folder dapat ditulis.']);
            exit;
        }
    }
    ...
    // Query INSERT
    if ($has_kategori) {
        $query = "INSERT INTO berita SET judul='$judul', slug='$slug', isi='$isi', gambar='$gambar', kategori='$kategori', status='$status', author='$author', created_at=NOW()";
    } else {
        $query = "INSERT INTO berita SET judul='$judul', slug='$slug', isi='$isi', gambar='$gambar', status='$status', author='$author', created_at=NOW()";
    }
    
    if (mysqli_query($conn, $query)) {
        ob_clean();
        echo json_encode(['status' => 'success', 'message' => 'Berita baru berhasil diterbitkan dan disimpan!']);
        exit;
    } else {
        ob_clean();
...
<div class="container-fluid px-3 px-md-4">
    <div class="card custom-card">
        <div class="custom-card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h5 class="m-0 font-weight-bold text-dark" style="font-size: 1.25rem;"><i class="fas fa-feather-alt text-success me-2"></i> Tulis Berita Baru</h5>
                <small class="text-muted">Buat tulisan publikasi informasi, agenda kegiatan, atau pengumuman resmi</small>
            </div>
        ...
        <div class="custom-card-body">
            <form id="formBerita" autocomplete="off">
                <!-- Input hidden untuk menampung olahan base64 gambar -->
                <input type="hidden" id="gambar_base64" name="gambar_base64">
                
                <div class="row">
                    <!-- Kolom Kiri: Input Utama Formulir -->
                    <div class="col-lg-8">
                        <div class="form-group mb-4">
                            <label for="judul" class="form-label-custom">Judul Berita <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-custom" id="judul" name="judul" placeholder="Masukkan judul utama berita yang menarik...">
                        </div>
                        
                        <div class="form-group mb-4">
                            <label for="isi" class="form-label-custom">Konten / Isi Berita <span class="text-danger">*</span></label>
                            <textarea class="form-control form-control-custom" id="isi" name="isi" rows="12" placeholder="Tuliskan isi informasi detail naskah berita di sini..."></textarea>
                        </div>
                    </div>
                    
                    <!-- Kolom Kanan: Pengaturan Tambahan & File Upload -->
                    <div class="col-lg-4">
                        <?php if ($has_kategori): ?>
                        <div class="form-group mb-4">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select class="form-control form-control-custom" id="status" name="status">
                                ...
                                <option value="draft">📁 Draft (Simpan sebagai Arsip Internal)</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <!-- Area Dropzone/Uploader Custom -->
                            <div class="dropzone-area" onclick="document.getElementById('file_gambar').click()">
                                <i class="fas fa-cloud-upload-alt dropzone-icon" id="icon_upload_placeholder"></i>
                                <p class="mb-1 text-secondary font-weight-500" id="text_upload_placeholder" style="font-size: 0.85rem;">Klik untuk pilih file gambar</p>
                            ...
                            <div class="text-center">
                                <div class="preview-container">
                                    <button type="button" class="btn-remove-preview" id="btnRemovePreview" title="Hapus Gambar"><i class="fas fa-times"></i></button>
                                    <img id="preview_gambar" class="preview-image-wrapper" src="#" alt="Preview Cover">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 btn-action-group">
                    ...
                    <button type="submit" id="submitBtn" class="btn btn-save-custom"><i class="fas fa-paper-plane me-1"></i> Terbitkan Berita</button>
                </div>
            </form>
        </div>
    </div>
...
// Pemrosesan Preview Gambar Terpilih & Encode ke String Base64
fileGambar.addEventListener('change', function(e) {
    const file = e.target.files[0];
    if (file) {
        // Validasi Ukuran File (Opsional - Batas Aman 5MB)
        if (file.size > 5 * 1024 * 1024) {
            alert('⚠️ Ukuran berkas gambar terlalu besar! Maksimal batas ukuran file adalah 5MB.');
            this.value = '';
            return;
        }
        ...
        reader.onload = function(event) {
            ...
            // Perubahan Interface Visual Elemen Uploader
            previewGambar.style.display = 'block';
            btnRemovePreview.style.display = 'flex';
            dropzoneIcon.className = "fas fa-check-circle text-success dropzone-icon";
            dropzoneText.innerHTML = "Gambar berhasil dimuat!";
        }
        reader.readAsDataURL(file);
    }
});

// Aksi Pembatalan / Pembersihan File Gambar Preview
btnRemovePreview.addEventListener('click', function(e) {
    ...
    dropzoneText.innerHTML = "Klik untuk pilih file gambar";
});

// Sistem Pengiriman Data via Fetch API Ajax secara Aman
document.getElementById('formBerita').addEventListener('submit', function(e) {
    e.preventDefault();
    
    // Sinkronisasi data dinamis jika terintegrasi engine text editor eksternal
    if (typeof CKEDITOR !== 'undefined' && CKEDITOR.instances['isi']) {
        CKEDITOR.instances['isi'].updateElement();
    }
    
    // Tarik nilai value input elemen HTML secara manual (Bypass kendala atribut name mismatch)
    ...
    // Validasi Masukan Wajib di Sisi Frontend Browser
    if (!judul) {
    ...
    // Konstruksi data payload menggunakan objek FormData secara eksplisit
    ...
    // Eksekusi pengiriman data asinkronus ke server
    ...
    .then(text => {
        try {
            ...
            console.error("Respon Ilegal Server:", text);
            throw new Error('Format respon server rusak atau tidak valid.');
        }
    })
    .then(data => {
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<i class="fas fa-paper-plane me-1"></i> Terbitkan Berita';
